...
- NOTE: `#ffadb5` (web error-message) was intentionally kept distinct from mobile `danger #ffabb2`. **Resolved**: web `.error-message` now uses the shared `--t-danger`; the web-only `--t-error` was removed — the palette is byte-identical across platforms.
...
## Session: Full-disk storage visibility + space cleaning (mobile, PROGRESS follow-up)

- **The mismatch**: iOS Settings → the app showed ~8 GB, while the app's Clear button only ever tracked ~253 MB. Cause: the button only deletes app-tracked paths (`clippress/` outputs, cache orphans matching `\.compressed\.h2\d+\.crf\d+\.mp4$`, `DocumentPicker/`), but iOS Settings reports the whole sandbox — everything in `Documents`, `Library/Caches`, `Library/Application Support`, and `tmp/`, plus the app binary. Sources copied in by the document picker + `tmp/` leftovers from uploads/downloads can be many GB. ("Offload App" also doesn't show for dev/ad-hoc-signed builds.)
- **`inspectStorage()`** (`mobile/src/compressionService.ts`): `StorageEntry`/`StorageRoot` types, `SANDSCAN_DEPTH = 5`, `deriveRoots()` (computes `sandbox/<App>/tmp/` + `Library/Application Support/` by string-slicing the `Caches/` path — expo-file-system only exposes `documentDirectory` + `cacheDirectory`), `dirSize()` (recursive, depth-capped), `listRoot()` (per-root sorted entries + size), and `inspectStorage()` returning all four roots: `Documents`, `Caches (Library)`, `Application Support`, `tmp (temporary)`. `logStorageState()` now also prints the per-root breakdown + grand total at launch.
- **Clear now sweeps `tmp/`** too (`clearStoredFiles()`): outputs + cache orphans + `DocumentPicker/` + everything in the app's `tmp/`. **`Application Support` is deliberately NOT cleared** — AsyncStorage (saved server URL) and Expo-managed data live there; a blanket wipe would break the app. If the inspector shows it's the hog, the next step is scoped sub-folder cleanup.
- **`StorageInspector` component** (`mobile/src/components/StorageInspector.tsx`): a separate card (NOT inside the empty state) rendering the per-root table + `Total on device`, a `Stored: N outputs (X) · N cached (Y)` summary, and the **Clear stored files** button. `EmptyState` is back to just the "No videos imported yet." banner. `useStorage` exposes `inspection` and re-runs `inspectStorage()` after clearing.
- **Verification**: `npx tsc --noEmit` ✓, `npx expo export --platform ios` ✓. (Expo Go on device still pending for the actual 8 GB breakdown.)
...
- Confirm on-device via Expo Go where the ~8 GB actually lives (the `StorageInspector` card + launch logs print the per-root breakdown). If `Application Support` dominates, add scoped cleanup of the known safe sub-folders there (Expo caches), keeping AsyncStorage. If `Documents` dominates, it's likely picker source copies — consider deleting the source after a successful convert (keep only the compressed output).